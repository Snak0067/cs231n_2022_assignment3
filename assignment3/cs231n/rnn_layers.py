...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    raw_h = np.dot(x, Wx) + np.dot(prev_h, Wh) + b
    next_h = np.tanh(raw_h)
    cache = (x, prev_h, Wx, Wh, raw_h)
...
def rnn_step_backward(dnext_h, cache):
    ...
    x, prev_h, Wx, Wh, raw_h = cache
    # Backprop step 2:
    dforward = (1 - np.tanh(raw_h) ** 2) * dnext_h  # [NxH]

    # Backprop step 1:
    dx = np.dot(dforward, Wx.T)  # [NxD]
    dWx = np.dot(x.T, dforward)  # [DxH]
    dprev_h = np.dot(dforward, Wh.T)  # [DxH]
    dWh = np.dot(prev_h.T, dforward)  # [HxH]
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
        if t == 0:
            h[:, t, :], forward_step_cache[t] = rnn_step_forward(x[:, t, :], h0, Wx, Wh, b)
        else:
            h[:, t, :], forward_step_cache[t] = rnn_step_forward(x[:, t, :], h[:, t - 1, :], Wx, Wh, b)
...
def rnn_backward(dh, cache):
    ...
    for t in reversed(range(T)):
        dh_ag = dh[:, t, :] + dprev_h
        dx_t, dprev_h, dWx_t, dWh_t, db_t = rnn_step_backward(dh_ag, forward_step_cache[t])
        dWx += dWx_t
        dWh += dWh_t
        db += db_t
        dx[:, t, :] = dx_t
